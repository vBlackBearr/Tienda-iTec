...
async def updateRawMaterial(raw_material_id, updated_raw_material):
    async with httpx.AsyncClient() as client:
        response = await client.put(f"http://localhost:8000/backend/raw_materials/{raw_material_id}",
...
async def updateProductSale(product_sale_id, updated_product_sale):
    async with httpx.AsyncClient() as client:
        response = await client.put(f"http://localhost:8000/backend/products_sales/{product_sale_id}",
...
async def getSession(credentials):
    async with httpx.AsyncClient() as client:
        response = await client.post("http://localhost:8000/backend/session", json=credentials)
